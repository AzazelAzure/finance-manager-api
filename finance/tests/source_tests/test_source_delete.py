...
class SourceDeleteTestCase(SourceBase):
    def test_delete_collection_body_success(self):
        ...
        response = self.client.delete(self.detail_url(expected["source"]))
    ...
    def test_delete_nonexistent_source_rejected(self):
        response = self.client.delete(self.detail_url("missing"))
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)

    def test_delete_unknown_rejected(self):
        response = self.client.delete(self.detail_url("unknown"))
