...
# Basic Transaction Creation Tests
    ...
    def test_snapshot_total_monthly_spending_current_month_expense(self):
        ...
        self.assertGreaterEqual(
            Decimal(str(spent)),
            Decimal("0.01"),
            msg="total_monthly_spending should include today's expense (positive magnitude)",
        )
    ...
    def test_transfer_batch_without_tags_or_date_returns_201(self):
        """
        ...
        leaks = response.data.get("snapshot", {}).get("total_leaks")
        self.assertIsNotNone(leaks)
        self.assertEqual(Decimal(str(leaks)), Decimal("0.00"))

    def test_calc_leaks_balanced_signed_pair_is_zero(self):
        """XFER_IN positive + XFER_OUT negative with same magnitude nets to zero leaks."""
        from types import SimpleNamespace

        from finance.logic.fincalc import Calculator

        bc = self.profile.base_currency
        fc = Calculator(self.profile)
        txs = [
            SimpleNamespace(tx_type="XFER_IN", currency=bc, amount=Decimal("14")),
            SimpleNamespace(tx_type="XFER_OUT", currency=bc, amount=Decimal("-14")),
        ]
...
# Data Permutation Tests
 ...
 # Tests For Different Data Types for Amount
    ...
    def test_post_with_recurring_bill_due_this_month_does_not_500(self):
        """
        Regression: _tx_snapshot_handler passed a dict to calc_sts, which iterates keys (str)
        and raised AttributeError when any unpaid recurring bill fell in the current month.
        """
        UpcomingExpense.objects.create(
            uid=self.profile.user_id,
            name="regression-recurring-bill",
