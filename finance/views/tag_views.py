...
@extend_schema_view(    
    post=extend_schema(
        ...
        description="Add one or more tags.\n"
                    "`tags` may be an array of strings, a single string (multi-word allowed), "
                    "or a comma-separated string. Tags are stored lowercase; duplicates are rejected.",
    ...
    def post(self, request):
        ...
        if raw is None:
            ...
            if not s:
                ...
                payload["tags"] = [p.strip() for p in s.split(",") if p.strip()]
            else:
                payload["tags"] = [s]
        else:
            payload["tags"] = [raw]
