...
def _validate_category(uid, data: dict, cat_check: set, patch: bool):
    ...
    data["name"] = name
    existing_lower = {str(c).lower() for c in cat_check}
    if not patch and name.lower() in existing_lower:
        raise ValidationError("Category already exists")
    if patch and name.lower() in _DEFAULT_CATEGORIES:
...
def CategoryGetValidator(func):
    ...
    def _wrapped(uid, cat_name: str, *args, **kwargs):
        ...
        slug = str(cat_name).strip()
        if slug.lower() in _DEFAULT_CATEGORIES:
            raise ValidationError("Cannot modify default category")
        checked = categories.filter(name__iexact=slug).first()
        if not checked:
            raise ValidationError("Category does not exist")
        kwargs["categories"] = categories
        kwargs["checked"] = checked
        kwargs["patch"] = True
        return func(uid, checked.name, *args, **kwargs)
