...
def update_category(uid, cat_name, data, *args, **kwargs):
    ...
    if new_name.lower() in {"expense", "income", "transfer"}:
        raise ValidationError("Cannot use default category")
    existing = list(Category.objects.for_user(uid).values_list("name", flat=True))
    existing_lower = {str(n).lower() for n in existing}
    if new_name.lower() != checked.name.lower() and new_name.lower() in existing_lower:
