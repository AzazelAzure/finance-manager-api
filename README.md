...
Status legend: `GA` (production-ready), `Beta` (usable, actively changing), `Planned` (not implemented).

| Area | Status | Notes |
|---|---|---|
| Auth + JWT | GA | `/api/token/`, `/api/token/refresh/`, JWT-protected finance endpoints |
| User account | GA | Create/read/password update/delete for current user context |
| App profile + snapshot | GA | Profile settings and computed monthly snapshot routes |
| Transactions | GA | Create/list/get/patch/delete; fidelity constraints enforced |
| Upcoming expenses | GA | Create/list/get/put/patch/delete |
| Sources | GA | Create/list/get/put/patch/delete; reserved `unknown` protections |
| Categories | GA | Create/list/get/put/patch/delete |
| Tags | GA | List/create/put/patch/delete with mapping semantics |
| OpenAPI docs | GA | `/api/schema/` and `/api/docs/` wired via `drf-spectacular` |

## Known Gaps / Follow-ups

- Versioned API namespace is not yet exposed as `/v1/...` paths; versioning is currently tracked by release tags and compatibility docs.
- Dependency locking can be tightened further (full transitive lock strategy documented in `docs/DEPENDENCY_LOCKFILES.md`).

## Documentation Standards (Current Phase)

- Non-encrypted phase policy: keep implementation docs explicit and developer-focused.
- Public APIs, request/response shapes, invariants, and runbooks should be documented in-repo and updated with each behavior change.
- Future encrypted/private modules can move sensitive internals to restricted docs, while preserving public integration contracts.
...
# optional: refresh exchange rates
...
Dependencies are managed with `pyproject.toml` + `uv.lock`.
