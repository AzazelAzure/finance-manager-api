...
### Fixed
- **Profile snapshot source hydrate (HFM-SRC-1):** `user_get_totals` materializes `transactions_for_month` after aggregations and runs `resolve_transactions_for_api` so API `source` is the payment-source display name (parity with `get_transactions`), not stored `source_id`.
- **Profile snapshot source filter (HFM-B2):** `user_get_totals` resolves payment-source display names to `source_id` before `apply_transaction_filters`, matching `get_transactions` parity. Unknown source strings remain literal and return empty filtered results.
- **Migration 0019 BalanceSnapshot dedup (T04):** Harden `backfill_name_linkage_surfaces` to merge/delete collision-eligible rows (same `uid`/`snapshot_date`, display names collapsing to one `source_id`) before save — fixes VPS `IntegrityError` on `unique_balance_snapshot_per_source_day`. Migration test fixture added.
- **Payment source linkage standardization (T01):** Added stable `PaymentSource.source_id` and migrated transaction, balance snapshot, savings goal, and `spend_accounts` linkage to store `source_id` internally while API JSON continues to expose display names. Renaming a payment source no longer breaks linked rows on the four surfaces.
...
- **Bill recurrence engine (T02):** `bill_recurrence.py` advances due dates from explicit `cadence` (semimonthly uses 1st/15th anchors; monthly/quarterly/annual use `relativedelta`). Unit tests in `test_bill_recurrence.py`.
- **Bill recurrence engine (T03):** Upcoming-expense serializers expose `cadence` and `custom_interval_days` with validation (`custom` requires positive days; non-custom clears interval days). Integration tests in `test_expense_cadence_api.py`.
...
- **F-005 savings goal API (T02):** CRUD at `/finance/savings-goals/` and `/finance/savings-goals/{id}/` with computed `per_cycle_required` based on profile pay-cycle frequency; cross-user access returns 404; `source` validated against the requesting user's payment sources. Bugbot follow-up: reject negative/non-finite `target_amount`/`current_amount`, reject null/blank `name` with 400 (was 500), and round `per_cycle_required` up so a positive remaining balance never reports 0.00.
...
- **F-004 API serializers (T03):** Profile GET/PATCH and expense CRUD expose pay-cycle and bill-realism fields with validation (pay_cycle requires frequency + anchor; partial amount ≤ bill total).
- **F-004 STS pay-window engine (T04):** `pay_cycle.py` computes profile pay windows; snapshot STS and `total_remaining_expenses` use pay-cycle windows when configured and count partial-payment obligations instead of full bill totals.
